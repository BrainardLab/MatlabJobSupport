...
[dockerScriptPath, dockerScriptBase] = fileparts(dockerScriptFile);

% default ssh script name based docker script name
if isempty(sshScriptFile)
    sshScriptFile = fullfile(dockerScriptPath, [dockerScriptBase '-ssh.sh']);
end
...
try
    ...
    remoteScriptFile = sprintf('%s-%s.sh', randomFolder, dockerScriptBase);
    fprintf(fid, 'scp %s %s \\\n', scpPortOption, idOption);
    fprintf(fid, '  "%s" "%s@%s:%s" \n', dockerScriptFile, user, host, remoteScriptFile);
